...
void OperatingSystem_SaveContext(int PID) {
	...
	// Save ticks dormido
	processTable[PID].ticksDormido = Clock_GetTime();
...
void OperatingSystem_HandleClockInterrupt(){ 
	...
	while (processTable[process].whenToWakeUp == numberOfClockInterrupts) {
		int prioridadAntigua = processTable[process].priority;
		int descuento = Clock_GetTime() - processTable[process].ticksDormido;
		int nuevaPrioridad = prioridadAntigua - descuento;
		OperatingSystem_ShowTime(EXAM);
		if (nuevaPrioridad >= 0) {
			processTable[process].priority = nuevaPrioridad;
			ComputerSystem_DebugMessage(140, EXAM, process,programList[processTable[process].programListIndex]->executableName, prioridadAntigua, nuevaPrioridad);
		} else {
			processTable[process].priority = 1;
			ComputerSystem_DebugMessage(140, EXAM, process,programList[processTable[process].programListIndex]->executableName, prioridadAntigua, 1);
		}
